...
$(document).ready(function() {
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			if(categoriesCode == "") {
				$("#categoriesCode").after('<p class="text-danger">Brand Name field is required</p>');
				$('#categoriesCode').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#categoriesCode").find('.text-danger').remove();
				// success out for form 
				$("#categoriesCode").closest('.form-group').addClass('has-success');	  	
			}
			...
			if(categoriesCode && categoriesName && categoriesStatus) {
				var form = $(this);
				// button loading
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function() {
					...
					if(categoriesCode == "") {
						$("#editCategoriesCode").after('<p class="text-danger">Brand Name field is required</p>');
						$('#editCategoriesCode').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editCategoriesCode").find('.text-danger').remove();
						// success out for form 
						$("#editCategoriesCode").closest('.form-group').addClass('has-success');	  	
					}
