...
$sql = "SELECT product.product_id, product.product_name, product.product_image, product.brand_id,
 		...
 		brands.brand_code, categories.categories_code FROM product 
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[7] == 1) {
 		// activate member
 		$active = "<label class='label label-success'>ยืมได้</label>";
 	} elseif ($row[7] == 2) {
 		// deactivate member
 		$active = "<label class='label label-danger'>ถูกยืมไป</label>";
 	} elseif ($row[7] == 3) {
		// deactivate member
		$active = "<label class='label label-warning'>กำลังซ่อมแซม</label>";
	} elseif ($row[7] == 4) {
		// deactivate member
